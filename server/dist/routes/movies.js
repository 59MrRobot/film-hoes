...
router.get('/keywords', async (req, res) => {
    try {
        ...
        const response = await fetch(`${TMDB_BASE_URL}/search/keyword?query=${encodeURIComponent(query)}&api_key=${TMDB_API_KEY}`);
        if (!response.ok)
...
router.get('/search', async (req, res) => {
    try {
        ...
        const response = await fetch(`${TMDB_BASE_URL}/search/movie?query=${encodeURIComponent(query)}&api_key=${TMDB_API_KEY}&include_adult=false&language=en-US&page=1`, {
            headers: {
        ...
        const movies = await Promise.all(searchResults.map(async (movie) => {
            let director = null;
            try {
                const creditsRes = await fetch(`${TMDB_BASE_URL}/movie/${movie.id}/credits?api_key=${TMDB_API_KEY}`);
                if (creditsRes.ok) {
                    const creditsData = await creditsRes.json();
                    director = creditsData.crew?.find((member) => member.job === 'Director')?.name || null;
                }
            }
            catch (err) {
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const cast = data.credits?.cast?.slice(0, 10).map((member) => member.name) || [];
        const youtubeVideos = data.videos?.results?.filter((v) => v.site === 'YouTube') || [];
        const trailer = youtubeVideos.find((v) => v.type === 'Trailer') || youtubeVideos[0] || null;

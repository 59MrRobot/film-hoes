...
router.get('/latest', async (req, res) => {
    try {
        const latestWeek = await db_1.prisma.week.findFirst({
            orderBy: { id: 'desc' },
            include: {
                nominations: {
                    include: {
...
router.get('/', async (req, res) => {
    try {
        ...
        const nominations = await db_1.prisma.nomination.findMany({
...
router.get('/history', async (req, res) => {
    try {
        const latestWeek = await db_1.prisma.week.findFirst({ orderBy: { id: 'desc' } });
        const historicalWeeks = await db_1.prisma.week.findMany({
            where: {
                isActive: false,
...
router.post('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const userNominations = await db_1.prisma.nomination.count({
        ...
        const existingNomination = await db_1.prisma.nomination.findFirst({
            where: { weekId: week.id, tmdbMovieId },
        });
        if (existingNomination) {
            return res.status(400).json({ error: 'This movie is already nominated this week' });
        }
        const nomination = await db_1.prisma.nomination.create({
            data: {
                weekId: week.id,
                userId,
                tmdbMovieId,
...
router.delete('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const week = await getActiveWeek();
        const totalVotesThisWeek = await db_1.prisma.vote.count({
            where: {
                nomination: {
                    weekId: week.id
                }
        ...
        await db_1.prisma.nomination.delete({
            where: { id: nominationId },
        });
...
router.put('/active-week/theme', auth_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user.isAdmin) {
        ...
        const week = await getActiveWeek();
        const updatedWeek = await db_1.prisma.week.update({
            where: { id: week.id },
            data: { theme },
        });
...
router.post('/start-week', auth_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user.isAdmin) {
        ...
        const newWeek = await db_1.prisma.week.create({
